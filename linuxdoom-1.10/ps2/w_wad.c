...
void W_AddFile(char *filename)
{
    ...
    // Fill in lumpinfo
    lumpinfo = realloc(lumpinfo, (numlumps+locallmps) * sizeof(lumpinfo_t));
...
void W_InitLocalLumps(void)
{
    lumpinfo_t *lump_p = &lumpinfo[numlumps];
    for (int i = 0; i<locallmps; i++)
    {
        lump_p->handle = 0xFFFFFFFF;
        lump_p->position = 0;
        lump_p->size = 0;
        strncpy(lump_p->name, locals[i].name, 8);
        lumpcache[numlumps+i] = locals[i].ptr;
        lump_p++;
    }
}
...
void W_InitMultipleFiles(char **filenames)
{
    ...
    // set up caching
    size = (numlumps+locallmps) * sizeof(*lumpcache);
...
int W_CheckNumForName(char *name)
{
    ...
    lump_p = lumpinfo + numlumps + locallmps;
               ...
               int tag)
{
    ...
    if ((unsigned)lump >= numlumps+locallmps)
        I_Error("W_CacheLumpNum: %i >= numlumps", lump);
    else if (lump >= numlumps)
...
W_CacheLumpName(char *name,
                int tag)
{
    int ret = W_GetNumForName(name);
    return W_CacheLumpNum(ret, tag);
}
