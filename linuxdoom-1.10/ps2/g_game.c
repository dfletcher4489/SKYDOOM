...
int G_PlayerGetNextWeapon(int shift)
{
    ...
    while (comp != currWeapon)
    {
        ...
        if ((player->weaponowned[comp] || (shotgunhack && comp == wp_shotgun)) && !(sharewareVersion && (comp == wp_plasma || comp == wp_bfg)))
            break;
            ...
            mapthing_t *mthing)
{
    ...
    if (players[consoleplayer].viewz != 1)
        S_StartSound(mo, sfx_telept, MAX_SOUND_COUNT); // don't start sound on first frame
