...
// defaulted values
//
int lookSensitivity; // has default
...
#define CHEATSTRINGSIZE SAVESTRINGSIZE

int cheatStringEnter;
int cheatCharIndex;
int cheatCharNewIndex;
char cheatgamestring[CHEATSTRINGSIZE];
...
void M_QuickSave(void)
{
    if (!usergame)
    {
        S_StartSound(NULL, sfx_oof, MAX_SOUND_COUNT);
        return;
    }
...
void M_DrawMainMenu(void)
{
    if (!paused)
        ...
        V_DrawPatchDirect(viewwindowx + (scaledviewwidth - 68) / 2,
                          4, 0, W_CacheLumpName("M_PAUSE", PU_CACHE));
}
...
void M_QuitResponse(int ch)
{
    ...
    {
        if (gamemode == commercial)
            S_StartSound(NULL, quitsounds2[(gametic >> 2) & 7], MAX_SOUND_COUNT);
        else
            S_StartSound(NULL, quitsounds[(gametic >> 2) & 7], MAX_SOUND_COUNT);
        // I_WaitVBL(105);
    }
    I_Quit();
}
...
{
    choice = 0;
    image->lod.mag_filter = detailLevel;
    image->lod.min_filter = detailLevel;
...
boolean M_Responder(event_t *ev)
{
    ...
    if (saveStringEnter)
    {
        ...
        case KEY_UPARROW:
            if (checkCount(saveInputCharIndex - 1))
                saveInputCharIndex -= 1;
            else
                saveInputCharIndex = 35;
            break;
        case KEY_DOWNARROW:
            if (checkCount(saveInputCharIndex + 1))
                saveInputCharIndex += 1;
        ...
        case KEY_ENTER:
            ch = eligible[saveInputCharIndex][0];
            if (ch != 32)
    ...
    if (cheatStringEnter)
    {
        ...
        case KEY_SELECT:
            ...
            if (cheatgamestring[0])
            {
                // M_DoSave(saveSlot);
                convertCheatString();
                ST_checkCheat(cheatgamestring);
                memset(cheatgamestring, 0, CHEATSTRINGSIZE);
                cheatCharNewIndex = 0;
                cheatCharIndex = 0;
            }
            break;
        ...
        case KEY_UPARROW:
            if (checkCount(cheatCharNewIndex - 1))
                cheatCharNewIndex -= 1;
            else
                cheatCharNewIndex = 35;
            break;
        case KEY_DOWNARROW:
            if (checkCount(cheatCharNewIndex + 1))
                cheatCharNewIndex += 1;
        ...
        case KEY_ENTER:
            ch = eligible[cheatCharNewIndex][0];
            if (ch != 32)
                if (ch - HU_FONTSTART < 0 || ch - HU_FONTSTART >= HU_FONTSIZE)
                    break;
            if (ch >= 32 && ch <= 127 &&
                cheatCharIndex < CHEATSTRINGSIZE - 1 &&
                M_StringWidth(cheatgamestring) <
                    (CHEATSTRINGSIZE - 2) * 8)
            {
                cheatgamestring[cheatCharIndex++] = ch;
                cheatgamestring[cheatCharIndex] = 0;
            }
            break;
        }
    ...
    if (!menuactive)
        ...
        case KEY_MINUS: // Screen size down
            if (automapactive || chat_on)
                return false;
            M_SizeDisplay(0);
            S_StartSound(NULL, sfx_stnmov, MAX_SOUND_COUNT);
            return true;
    ...
    case KEY_DOWNARROW:
        ...
        do
        {
            ...
            if (currentMenu->menuitems[itemOn].status != -1)
                S_StartSound(NULL, sfx_pstop, MAX_SOUND_COUNT);
    ...
    case KEY_ENTER:
            ...
            currentMenu->menuitems[itemOn].status)
        {
            ...
            else
            {
                ...
                S_StartSound(NULL, sfx_pistol, MAX_SOUND_COUNT);
            }
        }
        return true;
    ...
    default:
        for (i = itemOn + 1; i < currentMenu->numitems; i++)
            if (currentMenu->menuitems[i].alphaKey == ch && currentMenu->menuitems[i].status != -1)
            {
                itemOn = i;
                S_StartSound(NULL, sfx_pstop, MAX_SOUND_COUNT);
                return true;
...
void M_Drawer(void)
{
    ...
    if (max > 6 && currentMenu == &MainDef)
    {
        drawSkullLine = 5;
    }
    else
    {
        drawSkullLine = itemOn;
    }

    // DRAW SKULL
    V_DrawPatchDirect(x + SKULLXOFF, currentMenu->y - 5 + drawSkullLine * LINEHEIGHT, 0,
...
void M_Cheats(int choice)
{
    if (!usergame)
    {
        M_StartMessage("You need to be in game to use cheats!", NULL, false);
        return;
    }
